...
def calculate_errors(true_data, imputed_data, mask):
    ...
    print(selected_imputed_data_np.shape, true_data_cropped_np.shape, mask_cropped_np.shape)
...
def compare_models(data, models, parameters, epochs=100, batch_size=32, missing_mode='continuous', missing_rate=0.2,
                   device=None, n_features=2, n_steps=64, patience=5):
    results = {}
...
def main():
    ...
    # 比较模型
    results = compare_models(data, models, parameters, epochs=2, batch_size=32, device=device,
                             missing_mode='continuous', missing_rate=0.2, n_features=2, n_steps=64, patience=1)
